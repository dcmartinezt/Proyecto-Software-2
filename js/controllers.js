                ...
                execute(function (resp) {
                    $scope.$apply(function () {
                        ...
                        if (resp.error) {
                            ...
                            $scope.messages = 'El perfil ha sifo modificado';
                            $scope.alertStatus = 'Completado';
                            $scope.submitted = false;
                            $scope.initialProfile = {
                ...
                execute(function (resp) {
                    $scope.$apply(function () {
                        ...
                        if (resp.error) {
                            ...
                            // The request has succeeded.
                            $scope.messages = 'Se ha realizado la reserva exitosamente : ' + resp.result.name;
            ...
            execute(function (resp) {
                $scope.$apply(function () {
                    ...
                    if (resp.error) {
                        // The request has failed.
                        var errorMessage = resp.error.message || '';
                        $scope.messages = 'Error al consultar la reservas : ' + errorMessage;
                        $scope.alertStatus = 'warning';
                        $log.error($scope.messages + ' filters : ' + JSON.stringify(sendFilters));
                    } else {
                        // The request has succeeded.
                        $scope.submitted = false;
                        $scope.messages = ' : ' + JSON.stringify(sendFilters);
                        $scope.alertStatus = 'success';
            ...
            execute(function (resp) {
                $scope.$apply(function () {
                    ...
                    if (resp.error) {
                        // The request has failed.
                        var errorMessage = resp.error.message || '';
                        $scope.messages = 'Error al consultar la reserva creada: ' + errorMessage;
                        $scope.alertStatus = 'warning';
                        ...
                        // The request has succeeded.
                        $scope.submitted = false;
                        $scope.messages = 'Consulta exitosa: Reservaciones creadas';
                        $scope.alertStatus = 'success';
                        $log.info($scope.messages);
            ...
            execute(function (resp) {
                $scope.$apply(function () {
                    if (resp.error) {
                        // The request has failed.
                        var errorMessage = resp.error.message || '';
                        $scope.messages = 'Error al consultar la reserva' + errorMessage;
                        $scope.alertStatus = 'warning';
                        $log.error($scope.messages);
                        ...
                        $scope.loading = false;
                        $scope.messages = 'Consulta tuvo éxito: Reservas a su nombre';
                        $scope.alertStatus = 'success';
                        $log.info($scope.messages);
                    }
                    $scope.submitted = true;
        ...
        gapi.client.conference.registerForConference({
            ...
            $scope.$apply(function () {
                ...
                if (resp.error) {
                    ...
                    if (resp.result) {
                        // Register succeeded.
                        $scope.messages = 'Reserva validada';
                        $scope.alertStatus = 'success';
                        $scope.isUserAttending = true;
                        $scope.conference.seatsAvailable = $scope.conference.seatsAvailable - 1;
                    } else {
                        $scope.messages = 'Error al registrar la reserva';
                        $scope.alertStatus = 'warning';
                    }
                }
            });
        ...
        gapi.client.conference.unregisterFromConference({
            ...
            $scope.$apply(function () {
                ...
                if (resp.error) {
                    // The request has failed.
                    var errorMessage = resp.error.message || '';
                    $scope.messages = 'Error al eliminar la reserva : ' + errorMessage;
                    $scope.alertStatus = 'warning';
                    $log.error($scope.messages);
                    if (resp.code && resp.code == HTTP_ERRORS.UNAUTHORIZED) {
                        oauth2Provider.showLoginModal();
                    ...
                    if (resp.result) {
                        ...
                        $scope.messages = 'Reserva cancelada';
                        $scope.alertStatus = 'success';
                        $scope.conference.seatsAvailable = $scope.conference.seatsAvailable + 1;
                        $scope.isUserAttending = false;
                        $log.info($scope.messages);
                        ...
                        var errorMessage = resp.error.message || '';
                        $scope.messages = 'Error al eliminar la reserva :' + $routeParams.websafeKey +
                            ' : ' + errorMessage;
                        $scope.messages = 'Error al eliminar la reserva :';
                        $scope.alertStatus = 'warning';
                        $log.error($scope.messages);
                    }
                }
...
conferenceApp.controllers.controller('RootCtrl', function ($scope, $location, oauth2Provider) {
            ...
            gapi.client.oauth2.userinfo.get().execute(function (resp) {
                $scope.$apply(function () {
                    if (resp.email) {
                        ...
                        $scope.alertStatus = 'success';
                        $scope.rootMessages = ' se ha iniciado sesion como' + resp.email;
                    }
                });
            });
        });
    ...
    $scope.signOut = function () {
        ...
        $scope.alertStatus = '';
        $scope.rootMessages = ' se ha cerrado Sesion';
    };
            ...
            oauth2Provider.signIn(function () {
                gapi.client.oauth2.userinfo.get().execute(function (resp) {
                    $scope.$root.$apply(function () {
                        ...
                        $scope.$root.rootMessages = 'Usuario registrado como:' + resp.email;
                    });
